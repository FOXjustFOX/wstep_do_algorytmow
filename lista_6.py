...
def hamming_distance(s1: str, s2: str) -> int:
    ...
    if len(s1) != len(s2):
        ...
        common_hamming = sum(c1 != c2 for c1, c2 in zip(s1[:min_length], s2[:min_length]))
...
def spell_checker(word: str, dictionary: list[str]) -> str | list[str]:
    ...
    if word in dictionary_set:
        ...
        distances = [
            (d, hamming_distance(word, d)) for d in same_length_words
        ]
...
FREQ_TABLES = {
    ...
    'polish': {  # Połączone 'a' z 'ą', 'c' z 'ć' itd.
        'a': 8.91 + 0.99, 'b': 1.47, 'c': 3.96 + 0.4, 'd': 3.25, 'e': 7.66 + 1.11,
        'f': 0.3, 'g': 1.42, 'h': 1.08, 'i': 8.21, 'j': 2.28, 'k': 3.51,
        'l': 2.1 + 1.82, 'm': 2.8, 'n': 5.72 + 0.2, 'o': 7.75 + 0.9,
        'p': 3.13, 'r': 4.69, 's': 4.32 + 0.66, 't': 3.98, 'u': 2.5,
        'w': 4.65, 'y': 3.76, 'z': 5.64 + 0.06 + 0.83,
    }
}
...
def detect_language(text: str, freq_tables: dict) -> str:
    ...
    for lang, lang_freq in freq_tables.items():
        ...
        error = sum(
            (text_freq.get(char, 0) - lang_freq.get(char, 0)) ** 2
...
def get_vowel_consonant_tables(freq_tables: dict) -> dict:
    ...
    for lang, table in freq_tables.items():
        ...
        consonant_total = sum(freq for char, freq in table.items() if char not in VOWELS)
        # Normalizacja do 100%
...
def detect_language_vowel_consonant(text: str, vc_freq_tables: dict) -> str:
    ...
    for lang, lang_vc_freq in vc_freq_tables.items():
        error = (
            (text_vc_freq["vowels"] - lang_vc_freq["vowels"]) ** 2 +
            (text_vc_freq["consonants"] - lang_vc_freq["consonants"]) ** 2
        )
        scores[lang] = error
...
if __name__ == "__main__":
    ...
    s1, s2 = "mama", "nawa"
    print(f"Zmodyfikowana odległość między '{s1}' a '{s2}': {modified_hamming_distance(s1, s2)}")
    s1, s2 = "kot", "kąt"
    print(f"Zmodyfikowana odległość między '{s1}' a '{s2}' (diakrytyki): {modified_hamming_distance(s1, s2)}")

    print("\n--- 1c) Słownik ---")
    my_dictionary = [
        "kot", "pies", "dom", "stół", "okno", "drzwi", "lampa", "kocur",
        "biurko", "lato", "zima", "wiosna", "jesień", "płaszcz", "kwiat"
    ]
    word1 = "kocur"
    ...
    text_pl = "W Szczebrzeszynie chrząszcz brzmi w trzcinie i Szczebrzeszyn z tego słynie."
    text_en = "The quick brown fox jumps over the lazy dog."
    text_de = "Franz jagt im komplett verwahrlosten Taxi quer durch Bayern."

    print("\n--- 2b) Detekcja języka (pełne tablice) ---")
    print(f"Tekst: '{text_pl[:30]}...' -> Wykryty język: {detect_language(text_pl, FREQ_TABLES)}")
    print(f"Tekst: '{text_en[:30]}...' -> Wykryty język: {detect_language(text_en, FREQ_TABLES)}")
    print(f"Tekst: '{text_de[:30]}...' -> Wykryty język: {detect_language(text_de, FREQ_TABLES)}")
    ...
    print(f"Tekst: '{text_pl[:30]}...' -> Wykryty język: {detect_language_vowel_consonant(text_pl, vc_tables)}")
    print(f"Tekst: '{text_en[:30]}...' -> Wykryty język: {detect_language_vowel_consonant(text_en, vc_tables)}")
    print(f"Tekst: '{text_de[:30]}...' -> Wykryty język: {detect_language_vowel_consonant(text_de, vc_tables)}")
    ...
    print(f"Dla '{s1}' i '{s2}', najdłuższy wspólny podciąg to '{substring}' (długość: {length})")

    print("\n--- 3b) Najdłuższy wspólny podciąg (z przerwami) ---")
    s1, s2 = "ApqBCrDsEF", "tABuCvDEwxFyz"
    subsequence, length = longest_common_subsequence(s1.lower(), s2.lower()) # ignorujemy wielkość liter
    print(f"Dla '{s1}' i '{s2}', najdłuższy wspólny podciąg to '{subsequence.upper()}' (długość: {length})")

    print("\n--- 3c) Złożoność obliczeniowa i Drzewa Trie (komentarz) ---")
    print("Algorytmy znajdowania najdłuższego wspólnego podciągu (Substring i Subsequence) oraz odległości Levenshteina,")
    print("zaimplementowane przy użyciu programowania dynamicznego, mają złożoność czasową O(m*n), gdzie m i n to długości")
    print("porównywanych ciągów. Jest to efektywne dla słów, ale może być zbyt wolne dla bardzo długich tekstów.")
    print("\nDrzewa Trie (prefiksowe) to struktura danych idealna do przechowywania słowników. Pozwalają na bardzo szybkie")
    print("sprawdzanie, czy słowo (lub jego prefiks) istnieje w zbiorze. Użycie Trie do znajdowania sugestii literówek")
    print("(np. w połączeniu z algorytmem odległości edycyjnej) jest znacznie bardziej wydajne niż iterowanie po całej liście słów.")
    print("Link z zadania: https://pl.wikipedia.org/wiki/Drzewo_trie")

    print("\n--- 3d*) Odległość Levenshteina ---")
    s1, s2 = "kot", "sok"
    print(f"Odległość Levenshteina między '{s1}' a '{s2}': {levenshtein_distance(s1, s2)}")
    s1, s2 = "płaszczka", "płaszczkaa"
    print(f"Odległość Levenshteina między '{s1}' a '{s2}': {levenshtein_distance(s1, s2)}")
